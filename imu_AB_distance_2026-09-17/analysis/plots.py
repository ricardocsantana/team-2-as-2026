...
def fig_reconstructed_paths():
    ...
    big.plot([w0[0, 0], w0[-1, 0]], [w0[0, 1], w0[-1, 1]], "--", lw=1.6, color=INK, zorder=3,
             label="straight line A–B — 54.6 m")
    ...
    for i, L in enumerate(T[0][1]["legs"]):
        ...
        d = w0[i + 1] - w0[i]
        perp = np.array([d[1], -d[0]])
        perp = perp / np.linalg.norm(perp) * (44 if i == len(T[0][1]["legs"]) - 1 else 30)
    ...
    fig.suptitle("Reconstructed paths — the route is 3 straight legs totalling 66.6 m; "
                 "A and B are 54.6 m apart",
